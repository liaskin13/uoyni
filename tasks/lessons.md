...
#### Codespace Commit Signing 403 "Author is invalid" — ROOT CAUSE FOUND 2026-08-19, prior entry below was WRONG

- **Root cause (verified by reproduction, both directions, 2026-08-19)**: GitHub Codespaces sets `user.name` at the system level (`/etc/gitconfig`) and its commit-signing service (`gh-gpgsign`) requires that value to **match the full/display name on the GitHub profile**, per GitHub's own docs (`docs.github.com/en/codespaces/troubleshooting/troubleshooting-gpg-verification-for-github-codespaces`, "conflicting git configuration" section). This Codespace's `/etc/gitconfig` had `user.name=liaskin13` (the login), but the GitHub profile's actual `name` field is `"lisa marie"` (confirmed via `gh api user`) — a real mismatch, not a token/session issue. `git commit --allow-empty -S` reproduced the 403 deterministically with `user.name=liaskin13`, then succeeded with `user.name` set to `"lisa marie"` at the repo level (`.git/config`, which overrides the system value) — confirmed both failing and passing cases directly, not inferred.
- **Why the previous "not a code problem, don't check identity" rule below was wrong**: it treated `user.name` as ruled out because it superficially "checked out" (matched local convention/login), without checking it against the actual thing Codespaces validates against — the GitHub profile's display name, not the git login. The mismatch was invisible unless you specifically diffed `git config user.name` against `gh api user --jq .name`.
- **Likely trigger**: L set a display name ("lisa marie") on the GitHub profile at some point; `/etc/gitconfig`'s `user.name` was set once at Codespace container creation from whatever the profile said then, and does NOT refresh on reconnect (explains the 2026-08-16 finding that "many reconnects didn't fix it" — reconnecting to the same container never touches `/etc/gitconfig`).
- **Fix applied**: `git config user.name "lisa marie"` at the repo level in `/workspaces/psoulc/.git/config`. This takes precedence over the stale system value and survives reconnects/container rebuilds (repo directory is a persistent volume). It does NOT propagate to a brand-new Codespace or a fresh clone — if the bug recurs elsewhere, check `git config user.name` against `gh api user --jq .name` first, and set the repo-level (or global `~/.gitconfig`) override to match.
- **How to apply**: If this exact error recurs (`gpg failed to sign the data... 403 | Author is invalid`) in this or another repo/Codespace, do NOT reach for `--no-gpg-sign` first. Run `git config user.name` vs `gh api user --jq .name` — if they differ, that's almost certainly it. Fix with `git config user.name "<profile display name>"` (repo or global level).
