...
**Confidence badge:** sits LEFT of the BPM digits (read-first, left-to-right scan order). Rectangular tag, 0px border-radius, `1px solid var(--border)`, `--surface` background — same anatomy as the BPM nixie. Label "CONF" in Chakra Petch 500 8-9px uppercase; number in Space Mono, percentage format ("62%"), no decimal. Hidden entirely (not dimmed) until a detection has actually run, and — as of the Dynamic Tempo Analysis update (2026-08-20, see below) — also hidden entirely (replaced by the ZONES badge) whenever the track has real measured tempo drift.

**Manual-range fix (2026-08-20):** the visibility gate previously suppressed CONF (and the octave control) whenever ANY manual `bpm_display` value existed, including a manual *range* like `"60-80"` (real rows in D's library) — that's a display bug, not intentional: a range isn't a single resolved BPM, so CONF/octave-control should still evaluate normally. Fixed via `hasCompleteManualBpm(track)`, which returns true only for a genuinely single resolved value.

**Tooltip (2026-08-20):** the badge now carries a `title` — dynamic-bucket tracks reading low get an honest explanatory line ("lower readings are expected for groove-based material — this measures rhythmic rigidity, not accuracy"); static-bucket tracks don't get that caveat (a low reading there is a real detection issue, not expected groove). See "Dynamic Tempo Analysis" below for the bucket system this draws from.
...
**Trigger threshold — revised 2026-08-20 (Dynamic Tempo Analysis, see below):** no longer a flat 0.6 — split into `OCTAVE_CONTROL_CONFIDENCE_THRESHOLD = { dynamic: 0.35, static: 0.6 }`, selected by the track's resolved genre bucket. Static-bucket tracks (HOUSE, TECHNO) keep the exact original 0.6 behavior, validated and built for that rigid-tempo material. Dynamic-bucket tracks (DYNAMIC, BREAKBEAT — D's catalog default) require a materially lower reading before suggesting a correction, since groove-based material naturally autocorrelates worse without being wrong (see Decisions Log). Also renders ONLY when the track has no measured ZONES drift (a segmented track has no single tempo to correct toward) and `manually_corrected` isn't already set.

**`manually_corrected` — a terminal state (2026-08-20):** `handleOctaveCorrect` now PATCHes `manually_corrected: true` alongside `detected_bpm`, kept deliberately separate from `detected_bpm_confidence` (which stays an honest measurement, never overloaded to also mean "a human intervened"). Once set, the octave control disappears and stays gone — it previously only hid once confidence happened to cross the threshold again, which never actually happened after a correction, so the button never disappeared at all.
...
**Pause-gate — confirmed settled (2026-08-19).** L flagged discomfort with pause-required editing during plan review (2026-07-24) but chose not to relitigate at the time; L confirmed directly this session ("I don't mind the pause gate for beatgrid") that this is no longer a live concern. The same pause-gate pattern was reused deliberately (not just copied) for the DYNAMIC/genre badge's double-click cycling below — same reasoning: a control that changes octave-correction visibility mid-set is a live-performance mis-click risk worth guarding, even though genre itself carries no audio-engine risk.

---

### Dynamic Tempo Analysis (Console — ZONES badge, DYNAMIC/genre badge)

Shipped 2026-08-20. Replaces the single-tempo CONF badge's implicit assumption ("every track has one true BPM") with rekordbox-grade Static/Dynamic analysis — genre-aware, matching the platform's standing bar (meet-or-beat rekordbox/Serato). Full context: `~/.claude/plans/wise-leaping-charm.md`.

**ZONES badge** (new) — same slot as CONF (left of BPM digits), same anatomy as the confidence badge family (rectangular tag, 0px radius, `1px solid var(--border)`, `--surface` background, Chakra Petch label + Space Mono value). Shows only when `beat_grid_points` has ≥2 real anchors: `"ZONES · N · 96–142 BPM"` (count + BPM range spanned). **Neutral chrome color** — per "Meters vs. Chrome" above, this is a structural readout (real measured drift), not a confidence measurement, so it does NOT inherit CONF's color exception. Carries a self-explanatory `title` tooltip ("Multiple tempo zones detected — this track's rhythm shifts, so there's no single BPM to rate confidence against"). When ZONES shows, CONF and the octave control do not render for that track — mutually exclusive, driven by data (segments exist or they don't), never by genre.

**Genre vocabulary (v1, deliberately small — 4 values):** `DYNAMIC` (default, groove-tolerant — D's catalog: funk/soul/disco/hip-hop/live mixed sets), `BREAKBEAT` (groove-tolerant — chopped/swung breaks), `HOUSE` (rigid-tempo), `TECHNO` (rigid-tempo). Maps to one of two threshold buckets (`dynamic`/`static`) — see octave-control threshold above. Unknown/future genres default to `dynamic` (the safe default, since D's catalog is the platform's core subject). Resolution chain: per-track `tempo_genre` → console-level default (`consoleDefaultGenre`, defaults to `DYNAMIC`) → hardcoded `DYNAMIC`.

**DYNAMIC/genre badge** (redesigned from the previously-undocumented `LIVE` badge, same slot/span after the BPM digits) — shows the track's resolved genre label always when a track is loaded (not gated on `isPlaying` — genre is a track classification, not a live measurement). While playing, always appends the live-detected BPM number (`DYNAMIC 128`, or `DYNAMIC —` before the live detector reaches confidence) — same always-show behavior the badge it replaces had, not gated on the live detector's own confidence threshold.

**Badge coexistence — "Genre Dims" (resolved via `/plan-design-review`, 2026-08-20):** when a track has both measured ZONES drift AND a manually-set genre, the genre badge drops to 40% opacity rather than rendering at full weight or being suppressed entirely — visibly secondary to the authoritative ZONES reading, still present as context. Validated via HTML wireframe built from this doc's real tokens (3 variants compared: Both Visible / Genre Dims / Genre Suppressed).

**Interaction — double-click OR keyboard (Enter/Space) cycles the vocabulary** (`DYNAMIC → BREAKBEAT → HOUSE → TECHNO → DYNAMIC`), **pause-gated**, matching the beatgrid anchor editor's precedent exactly (see above). The genre bucket feeds the octave-control threshold, so cycling mid-playback could make that button appear/disappear right under D's hand mid-set — a live-performance mis-click risk, not an audio-engine one. An attempted double-click/Enter while playing calls the existing `announceStatus("Pause to change tempo genre", "info")` mechanism (the general status readout, not bespoke on-canvas UI — this badge is a plain DOM span, not a canvas element like the beatgrid editor).

**Keyboard access + ARIA (2026-08-20):** the badge is a focusable control (`tabIndex={0}`, `role="button"`, `aria-label` stating the current genre, e.g. "Tempo genre: DYNAMIC. Press Enter to cycle.") — the console's first double-click-cycled control where the mouse-only interaction gates real behavior (the octave-control threshold), not just a label, so it ships with a non-mouse path from day one, matching the beatgrid anchor editor's own `[`/`]` keyboard nudge precedent.

**Discoverability hint:** hovering (or keyboard-focusing) the badge shows a small tag in the badge's own chrome color reading "DOUBLE-CLICK TO CYCLE" — same precedent as the hot-cue pads' hover-discoverable "DOUBLE-CLICK TO EDIT" tag (see Hot Cues below), positioned below the badge (not covering it — this badge has no glow-overlay `overflow:hidden` constraint forcing an inset cover the way hot-cue pads do).

**Settings-panel default (both `AdminSettings.jsx` and D's inline panel, "BEAT DETECTION" group):** a **cycle button** (not a dropdown or radio group) — a single `god-btn` showing the current console-level default, click cycles through the 4 values, mirroring the badge's own double-click-cycle interaction so both surfaces share one consistent interaction model rather than two different ones (pick-from-list vs. cycle-through) for the same underlying value.

**Sequence-guarded PATCHes:** both the genre-cycle handler and `handleOctaveCorrect` use a per-track-id request-sequence guard (not a single global counter — a correction/cycle on one track must never be discarded because a different track's action fired more recently) so an out-of-order network response can never silently overwrite a newer on-screen value with a stale one.
...
**Entry-point fix (2026-08-16):** the mechanism above shipped 2026-08-15 with one topic (BEATGRID) but stayed invisible — the placeholder just said "SEARCH VAULT," and the "⏎ HELP" chip only appears *after* you've already typed a matching keyword, so nobody could discover the feature existed without being told out-of-band. Placeholder is now `"SEARCH · OR TYPE HELP"`, and a `HELP` topic makes the whole system findable from itself. **Correction (2026-08-20):** `HELP`'s topic list is a hand-typed string in `helpTopics.js`, not actually generated from the registry — the "self-updating" description here was aspirational, not accurate; confirmed by reading the real code. Tracked in TODOS.md. Current topics: `BEATGRID`, `TAP`, `VALIDATION`, `HOTCUE`, `SHORTCUTS`, `VOID`, `TEMPO`, `HELP`.
