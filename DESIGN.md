...
8 pads per bank, 4 banks (A/B/C/D). Click an empty pad while playing to set a cue there; click a set pad to jump to it.

**Clearing (2026-08-16):** every occupied pad shows a small always-visible `×` in the top-right corner (`.arch-hotcue-clear`, 12×12px, `rgba(255,255,255,0.45)` idle → `#ff4444` on hover) — click it to clear that one cue, wired to the existing bank-agnostic `clearHotCue`. Not hover-gated — visible at rest, matching this session's broader "D needs it visible without hovering" theme, not just on interaction. Works on all 4 banks, including D for the first time (D-bank's double-click is fully claimed by the cue-rename feature below, so it never had any clear path before this).

Banks A/B/C also keep their older double-click-twice-within-3s confirm gesture (pulsing red border via `.clearing`) as a redundant power-user shortcut — undiscoverable on its own (no visual hint it exists before you try it), which is why the `×` exists now as the primary, visible path.

**Bank D — rename, not clear.** Double-click a D-bank pad opens an inline rename editor instead of arming a clear (D-bank cues can carry a short label, shown on the pad instead of its number). This is intentional and unchanged — the `×` coexists with it as a separate click target.
